...
class UserInfo extends REST_Controller
{
	...
	function changeavatar_post(){
		$postdata = file_get_contents("php://input");
		$request = json_decode($postdata);
		$data = $this->user_model->changeAva($request->username, $request->filename);
	...
	function removeavatar_get(){
		$username = $this->get("username");
		$data = $this->user_model->removeAva($username);
		$this->response($data);
	}
